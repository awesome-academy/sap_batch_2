*&---------------------------------------------------------------------*
*& Report ZSYR093_TASK15
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZSYR093_TASK15.

DATA: lt_airline TYPE TABLE OF zy093tsc,
      lt_flight TYPE TABLE OF zy093tf,
      lt_booking TYPE TABLE OF zy093tb.

" Clear old data before insert
CLEAR: lt_airline, lt_flight, lt_booking.
DELETE FROM zy093tsc.
DELETE FROM zy093tf.
DELETE FROM zy093tb.

*&---------------------------------------------------------------------*
*& Insert data into table AIRLINE
*&---------------------------------------------------------------------*
APPEND VALUE #(
...
INSERT zy093tsc FROM TABLE lt_airline.

IF sy-subrc = 0.
  WRITE: / 'Insert data to table AIRLINE zy093tsc successful.', sy-dbcnt, 'rows inserted.'.
ELSE.
  WRITE: / 'Error while inserting data to table AIRLINE zy093tsc.'.
ENDIF.
...
INSERT zy093tf FROM TABLE lt_flight.

IF sy-subrc = 0.
  WRITE: / 'Insert data to table FLIGHT zy093tf successful.', sy-dbcnt, 'rows inserted.'.
ELSE.
  WRITE: / 'Error while inserting data to table FLIGHT zy093tf.'.
ENDIF.
...
INSERT zy093tb FROM TABLE lt_booking.

IF sy-subrc = 0.
  WRITE: / 'Insert data to table BOOKING zy093tb successful.', sy-dbcnt, 'rows inserted.'.
ELSE.
  WRITE: / 'Error while inserting data to table BOOKING zy093tb.'.
ENDIF.